 ...
 * Canvas-based and theme-aware, it idles when no ring is alive, pauses off-screen
 * and in hidden tabs, and renders a still grid under prefers-reduced-motion.
 */
export function SonarGrid({
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      // Canvas is unavailable in non-browser renderers such as jsdom.
      return
    }
    if (!ctx) return
    ...
    const draw = (now: number) => {
      ...
      const lifetime = (Math.hypot(width, height) + o.ringWidth) / Math.max(o.speed, 1)
      ...
      const cols = Math.ceil(width / Math.max(o.spacing, 1)) + 1
      const rows = Math.ceil(height / Math.max(o.spacing, 1)) + 1
    ...
    const intersectionObserver = typeof IntersectionObserver !== "undefined"
      ...
      : null
    const mutationObserver = typeof MutationObserver !== "undefined" ? new MutationObserver(() => refreshRef.current()) : null
